...
# prepare.sh - Set up development environment for JavaScript WebAssembly template
...
VERBOSE=0
INSTALLED_TOOLS=()
LOCKFILE="/tmp/prepare-wasm-js-$(whoami).lock"
...
install_npm_packages() {
    ...
    # npm packages (jco, componentize-js) are installed via npm install
    if command_exists npm; then
        if [ -f "package.json" ]; then
            # Install all npm dependencies
            log_info "Running 'npm install' to install JavaScript packages..."
            npm install || {
        ...
        log_error "npm is required to install JavaScript packages"
        return 1
    fi
}
...
main() {
    echo "======================================"
    echo "JavaScript WebAssembly Template Setup"
    echo "======================================"
    echo ""
    ...
    # Install npm packages (jco and componentize-js)
    if [ "$CHECK_ONLY" -eq 0 ]; then
        if ! [ -f "node_modules/.bin/jco" ]; then
            install_npm_packages || {
                missing_deps=1
            ...
            log_success "jco is already installed in node_modules"
        fi
    else
